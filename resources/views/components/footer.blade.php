@php
    $footerMenu  = \App\Models\Menu::where('location', 'header')->with(['items' => fn($q) => $q->where('is_active', true)->whereNull('parent_id')->orderBy('order')])->first();
...
<footer style="background:#1a2535; color:#fff">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
            ...
            {{-- Navigation --}}
            <div>
                <h4 style="font-weight:600;margin-bottom:16px;color:rgba(255,255,255,.9);font-size:14px">{{ __('nav.navigation') }}</h4>
                <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:8px">
                    ...
                    @else
                        @foreach([
                            ...
                            [route('teachers.index'), __('nav.teachers')],
                            [route('blog.index'),     __('nav.blog')],
                            [route('gallery.index'),  __('nav.gallery')],
                            [route('contact.index'),  __('nav.contact')],
                        ] as [$href, $label])
                        <li>
                            <a href="{{ $href }}"
                               style="color:#94a3b8;font-size:14px;text-decoration:none;transition:color .15s ease"
            ...
            {{-- Contact --}}
            <div>
                <h4 style="font-weight:600;margin-bottom:16px;color:rgba(255,255,255,.9);font-size:14px">{{ __('contact.title') }}</h4>
                <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:10px">
                    @if($address = \App\Models\Setting::get('contact_address'))
                    <li style="display:flex;align-items:flex-start;gap:8px">
                        <svg style="width:16px;height:16px;color:#7EC8A4;flex-shrink:0;margin-top:2px" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <li style="display:flex;align-items:center;gap:8px">
                        <svg style="width:16px;height:16px;color:#7EC8A4;flex-shrink:0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
